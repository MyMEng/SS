...
We perform the SYN flood attack using \texttt{netwox} using the script we created (see \ref{script1}). We can then see on the victim machine that there are many connections opened at state ``SYN\_RECV'' from many random IP addresses (generated by \texttt{netwox}):\\
