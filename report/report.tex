...
\section{Conclusions}
Presented here attacks show how user can make harm to the system or gain \texttt{root} privileges by exploiting vulnerabilities in: \texttt{C} string formatting, \texttt{chroot} command/system call, and \texttt{C} buffer overflow. Majority of exploits presented here has already been accessed in never versions of Linux kernel or \texttt{C} language, nevertheless, many embedded systems like: internet cameras, NASes, and routers still use vulnerable software facilitating aforementioned attacks.
